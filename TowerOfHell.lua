...
local WSPEED, JPOWER = 16, 50
local saPButton = Box(loc, {"WalkSpeed:", "16"}, function(box, text)
...
local jpWButton = Box(loc, {"JumpPower:", "50"}, function(box, text)
    local nm = tonumber(box.Text)
    if nm then
        box.PlaceholderText = tostring(nm)
        box.Text = ""
        ...
        box.PlaceholderText = "50"
        box.Text = ""
        JPOWER = 50
    end
end, 3)

local DoubleJumpAmonut = 0
local djjButton = Box(loc, {"Double-Jump Amount:", "0"}, function(box, text)
    local nm = tonumber(box.Text)
    if nm then
        box.PlaceholderText = tostring(nm)
        box.Text = ""
...
local DKScript = false
local aasda1 = Button(loc, "Disable Kill Bricks: false", function(button)
	DKScript = not DKScript
    button.Text = "Disable Kill Bricks: " .. tostring(DKScript)
end, 1)

local AFObby = false
local abas = Button(loc, "Auto Farm: false", function(button)
	AFObby = not AFObby
    button.Text = "Auto Farm: " .. tostring(AFObby)
	if AFObby then
		if not DKScript then
			DKScript = true
			aasda1.Text = "Disable Kill Bricks: " .. tostring(DKScript)
		end
	end
end, 1)
...
local Connect = nil
local function NoVeloToggle(Bool)
	if Bool then
...
task.spawn(function()
	while task.wait() do
		local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
		if hrp and AFObby then
			local Center = workspace:FindFirstChild("tower") and workspace:FindFirstChild("tower"):FindFirstChild("sectionData") and workspace:FindFirstChild("tower"):FindFirstChild("sectionData"):FindFirstChild("finish") and workspace:FindFirstChild("tower"):FindFirstChild("sectionData"):FindFirstChild("finish"):FindFirstChild("center")
			if Center then
				NoVeloToggle(true)
				local Tween = TweenService:Create(hrp, TweenInfo.new(25, Enum.EasingStyle.Linear), {CFrame = Center.CFrame})
				Tween:Play()
				for i = 1, 25 do
					if AFObby then
						task.wait(1)
...
RunService.Stepped:Connect(function()
	...
	ReplicatedStorage.GameValues.globalJumps.Value = DoubleJumpAmonut
	ReplicatedStorage.GameValues.killbricksDisabled.Value = DKScript
